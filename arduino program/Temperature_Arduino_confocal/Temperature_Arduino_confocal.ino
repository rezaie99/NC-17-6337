...
unsigned long ONduration = 0; //total duration of state "on"
unsigned long ONstartTime = 0;//starting time of the last "on" period
...
float Vin = 5.0F;//V
float Rc[4] = {
  110000.0F,110000.0F,110000.0F,110000.0F}
...
void loop(void) 
{
  ...
  if((WindowSize - Output) < millis() - windowStartTime && Input<=Setpoint) // turn relay on, for a given part of the window, defined by the PID
  {
    if (state==LOW)//means that the state is changing, onset of ON state
    {
      ONstartTime=millis();
    }
    state=HIGH;
  ...
  {
    if (state==HIGH)//means that the state is changing, end of ON state
    {
      ONduration+=millis()-ONstartTime;//add the duration of last ON state to the total duration
    }
    state=LOW;
  }
