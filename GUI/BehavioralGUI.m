...
% Last Modified by GUIDE v2.5 10-Aug-2014 23:12:49
...
try
    ...
    set(temperatureArduino, 'Timeout', 0.01);
    pause(0.1)
...
% --- Executes on button press in TEMPERATURESET.
function TEMPERATURESET_Callback(hObject, eventdata, handles)
% hObject    handle to TEMPERATURESET (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
T = str2double(get(handles.WANTEDTEMP,'String'));
global temperatureArduino;
sendArduinoValue(temperatureArduino,T)
global setTemp;
setTemp=T;
global oneTime;
oneTime=0;
...
% --- Executes on button press in TEMPERATUREPAUSE.
function TEMPERATUREPAUSE_Callback(hObject, eventdata, handles)
% hObject    handle to TEMPERATUREPAUSE (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

currentString = get(hObject,'String');
if isequal(currentString, 'Pause')
    set(hObject,'String','Start again');
else
    set(hObject,'String','Pause');
end
global temperatureArduino;
sendArduinoValue(temperatureArduino,350);


% --- Executes on button press in FAN.
function FAN_Callback(hObject, eventdata, handles)
% hObject    handle to FAN (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global temperatureArduino;
sendArduinoValue(temperatureArduino,450);

% --- Executes on button press in TEMPERATURECONTROL.
function TEMPERATURECONTROL_Callback(hObject, eventdata, handles)
% hObject    handle to TEMPERATURECONTROL (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of TEMPERATURECONTROL
global manualTemperatureControl;
manualTemperatureControl = get(hObject,'Value');
global temperatureControlTimer;
global isTemperature;
if manualTemperatureControl==1 && isTemperature==1
    [a b ~] = uigetfile('.mat');
    if a~=0
        load([b a]);
        temperatureControlTimer = timer;
        temperatureControlTimer.StartDelay = 0;
        temperatureControlTimer.Period = 1/str2double(get(handles.TEMPERATUREFREQUENCY,'String'));
        temperatureControlTimer.ExecutionMode = 'fixedRate';
        temperatureControlTimer.BusyMode = 'queue';
        temperatureControlTimer.TimerFcn = {@setPIDoutput, outputTable};
    else
        set(hObject,'Value', 0);
        manualTemperatureControl = get(hObject,'Value');
    end
...
if manualTemperatureControl==1
   start(temperatureControlTimer);
end
...
if isShock==1
    analogIndex=0;
    AnalogInput(guivar.TimeLength*guivar.RepeatNm+(guivar.RepeatNm-1)*guivar.BreakTime,2)=0;
end
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
a = uigetdir(handles.defaultResultPath, 'Please select where the results will be saved');
if a~=0
...
% --- Executes on button press in LOGTEMPERATURE.
function LOGTEMPERATURE_Callback(hObject, eventdata, handles)
% hObject    handle to LOGTEMPERATURE (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
global temperatureAcquisitionTimer;
...
if isequal('Log T',s)
    ...
    freq=str2double(get(handles.TEMPERATUREFREQUENCY,'String'));
    
    TemperatureLog = zeros(freq*60*60*12,wantedTemperatureValues+1);
    
    
    temperatureAcquisitionTimer = timer;
    temperatureAcquisitionTimer.period = freq;
    temperatureAcquisitionTimer.BusyMode = 'queue';
    temperatureAcquisitionTimer.StartDelay = 0;
    temperatureAcquisitionTimer.ExecutionMode = 'fixedRate';
    temperatureAcquisitionTimer.TimerFcn = @temperatureLog;
    
    
    
    resultsFile = [get(handles.RESULTPATH,'String') '\results\'];
    if ~exist(resultsFile,'dir')
        mkdir(resultsFile);
    end
    
    resultsFile = [resultsFile, get(handles.Filename,'string')];
    
    if exist(resultsFile)>0
    ...
    start(temperatureAcquisitionTimer);
    set(hObject,'String', 'Stop Log T');
    
else
    
    stop(temperatureAcquisitionTimer);
    
    TemperatureLog(sum(TemperatureLog,2)==0,:)=[];%remove all rows full of 0
    ...
    save(resultsFile,'TemperatureLog');
    
    set(hObject,'String', 'Log T');
...
function startShockArduinoListening()
if ispc
...
if get(hObject,'Value')==1
    set(handles.time,'String',num2str(ceil(handles.Eshocksequenceduration/(100*60))+1));
    set(handles.TimInt,'String',num2str(1));
    set(handles.REPEAT,'String',num2str(1));
    set(handles.time,'Enable','off');
    set(handles.TimInt,'Enable','off');
...
load([PATHNAME FILENAME]);
handles.Eshocksequenceduration=dur;
global shockingArduinoTimer;
try
    stop(shockingArduinoTimer);
end
...
% startShockArduinoListening();

while AnalogInput(1)~=-42
    pause(0.5);
    disp('still no -42');
end
    pause(0.5);
